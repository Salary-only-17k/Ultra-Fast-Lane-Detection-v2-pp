...
def save_model(net_state_dict, optimizer_state_dict,acc,epoch, \
                save_path, \
                is_last):
    assert os.path.exists(save_path)
    if is_main_process():
        state = {'model': net_state_dict, 'optimizer': optimizer_state_dict,"acc":acc,'epoch':epoch}
        if is_last:
            model_path = os.path.join(save_path, f'model_last_{acc}.pth')
        else:
            model_path = os.path.join(save_path, f'model_best_{acc}.pth')
        torch.save(state, model_path)
...
def get_loader(cfg,is_val=False):
    if not is_val:
        if cfg.dataset == 'CULane':
        ...
        if cfg.dataset == 'CULane':
            val_loader = TrainCollect(cfg.batch_size, 4, cfg.data_root, os.path.join(cfg.data_root, 'list/val_gt.txt'), get_rank(), get_world_size(), 
                                    cfg.row_anchor, cfg.col_anchor, cfg.train_width, cfg.train_height, cfg.num_cell_row, cfg.num_cell_col, cfg.dataset, cfg.crop_ratio)
        else:
            raise NotImplementedError
        return val_loader 
