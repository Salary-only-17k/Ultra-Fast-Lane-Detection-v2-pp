...
class ClassifyLoss(nn.Module):   #! 车道线分类
    def __init__(self,n_lane,cls_lane, *args, **kwargs):
        super(ClassifyLoss, self).__init__()
        self.n_lane = n_lane
        self.cls_lane = cls_lane
        self.func = nn.CrossEntropyLoss()
    def forward(self, pre_lane_labels,lane_labels):
        batch_size = lane_labels.shape[0]
        pre_lane_labels = pre_lane_labels.reshape([batch_size,self.n_lane,self.cls_lane])
        # print("pre_lane_labels:  ",pre_lane_labels.shape)
        loss_v = 0
        for bs in range(batch_size):
            # print(pre_lane_labels[bs,...].shape,lane_labels[bs].shape)
            loss_v += self.func(pre_lane_labels[bs,...],lane_labels[bs])  # /self.n_lane
