from utils.loss import SoftmaxFocalLoss, ParsingRelationLoss, ParsingRelationDis, MeanLoss, \
            TokenSegLoss, VarLoss, EMDLoss, RegLoss,ClassifyLoss
...
def get_loss_dict(cfg):
    if cfg.dataset == 'CurveLanes':
        loss_dict = {
            ...
            'lanes_classify'],
            'op': [
                SoftmaxFocalLoss(2, ignore_lb=-1), #^ softmax软化分类loss 基于全局的
                ParsingRelationLoss(),   #^ 200*72*4⽹络输出loc_row
                ParsingRelationDis(), 
                SoftmaxFocalLoss(2, ignore_lb=-1), 
                torch.nn.CrossEntropyLoss(),  
                torch.nn.CrossEntropyLoss(), #^ 3*72*4⽹络输出exist_row，72*4的值不等于-1则为1，否则为0
                MeanLoss(), 
                MeanLoss(),
                ClassifyLoss],
            'weight': [1.0, cfg.sim_loss_w, cfg.shp_loss_w, 1.0, 1.0, 1.0, cfg.mean_loss_w, cfg.mean_loss_w,],
            'data_src': [('cls_out', 'cls_label'),          #^ 'cls_out'-pred['loc_row']        'cls_label'-data_label['labels_row']
            ('cls_out',),     # 相对位置#^ 'cls_out'-pred['loc_row']
            ('cls_out',),    # 相对距离#^ 'cls_out'-pred['loc_row']
            ('cls_out_col', 'cls_label_col'),               #^ 'cls_out_col'-pred['loc_col']    'cls_label_col'-data_label['labels_col']
            ('cls_out_ext', 'cls_out_ext_label'),           #^ 'cls_out_ext':pred['exist_row']  'cls_out_ext_label':cls_out_ext_label, 
            ('cls_out_col_ext', 'cls_out_col_ext_label') ,  #^ 'cls_out_ext':pred['exist_col']  'cls_out_col_ext_label':cls_out_col_ext_label, 
            ('cls_out', 'cls_label'),                        
            ('cls_out_col', 'cls_label_col'),    
            ("lane_labels","pre_lane_labels")              
            ],
        }
    else:
        raise NotImplementedError
