...
class LaneCls():  #!  here
    ...
    def update(self,predict,target):
        predict = converter_(predict)
        target = converter_(target)
        batchsize,n_lane,_ = predict.shape
        
        for bs in range(batchsize):
            self.correct += np.sum(np.argmax(predict[bs],axis=1)==target[bs])
        self.cnt += batchsize*n_lane
